...
Nonetheless, the horizon $T$ will be assumed strictly greater than $k$, such that all arms may be pulled at least once, thereby assuring that empirical means and perhaps also other statistics on which algorithms can rely are well-defined.
The goal is for the agent to maximise its cumulative rewards $\sum_{t=1}^T X_t$.
...
It is assumed that there be only one optimal arm, which is the arm with the highest mean reward.
While this is rarely a crucial property, it is still useful to avoid unnecessary complications and verbosity in the arguments and proofs that follow.
...
It is worth remarking that any probabilistic policy can be applied as a deterministic policy by defining the action to be the most likely action according to the policy.
...
It will be seen that an uninformed policy will generate $O(T)$ regret, while more clever solutions satisfy sublinear regret bounds.
