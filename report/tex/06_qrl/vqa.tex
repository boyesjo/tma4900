...
\subsection{Design}
Generally, VQAs start with defining a cost function, depending on some input data (quantum states) and the parametrised circuit, to be minimised with respect to the parameters of the quantum circuit.
...
Additionally, the cost function should be complicated enough to warrant quantum computation by not being easily evaluated on classical hardware, while still having few enough parameters to be efficiently optimised~\autocite{cerezo2021}.
...
A common approach is hardware-efficient ansätze, which, as the name implies, is a general term used for ansätze designed in accordance with the hardware properties of a given quantum computer, taking, for instance, the physical gates available into account, thereby minimising the circuit depth.
...
Not having to rely on finite differences is a major advantage, as the effects of small changes in parameters on the evaluated cost would quickly be drowned out in noise.
Furthermore, it may be used recursively to evaluate higher order derivatives, which allows the usage of more advanced optimisation methods like the Newton method that requires the Hessian, though this will naturally require further circuit evaluations.

Once the gradient is computed, the parameters can be updated with gradient descent methods.
...
A major obstacle for VQAs is that the gradients of their cost function can be exponentially small, a phenomenon known as barren plateaus.
Barren plateaus prohibit the optimisation from converging, as evaluating the gradient would need exponentially many circuit evaluations to produce sufficiently precise estimates.
...
It is therefore important that both the hardware and the ansatz be considered when designing a VQA.
