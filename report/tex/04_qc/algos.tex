...
The Deutsch-Jozsa algorithm~\autocite{deutsch1992}, while of no practical use, is proved to be exponentially faster than any deterministic classical algorithm\footnotemark~\autocite{simon1994}.x
...
\footnotetext{
    The provable exponential speed-up does not prove $\P \neq \BQP$, as the algorithm requires an oracle~\autocite{NEEDED}.
...
\end{center}
BPP is the class of problems that can be solved in polynomial time with a probabilistic algorithm, and $\PSPACE$ is the class of problems that can be solved in polynomial space.
