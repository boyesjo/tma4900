# forked from https://qiskit.org/documentation/_modules/qiskit/algorithms/amplitude_estimators/fae.html#FasterAmplitudeEstimation
...
class FasterAmplitudeEstimation(AmplitudeEstimator):
    ...
    def __init__(
        ...
        if shots is None:
            self._shots = (
                int(1944 * np.log(2 / delta)),
                int(972 * np.log(2 / delta)),
            )
        else:
            self._shots = (
                shots,
    ...
    def _cos_estimate(self, estimation_problem, k, shots):
        ...
        if self._sampler is not None:
            circuit = self.construct_circuit(
                estimation_problem, k, measurement=True
            )
            try:
            ...
            prob = 0
            for bit, probabilities in (
                result.quasi_dists[0].binary_probabilities().items()
            ):
            ...
            circuit = self.construct_circuit(
                estimation_problem, k, measurement=False
            )
            statevector = self._quantum_instance.execute(
                circuit
            ).get_statevector()
            ...
            for i, amplitude in enumerate(statevector):
                ...
                state = "".join(
                    [
                        full_state[i]
                        for i in estimation_problem.objective_qubits
                    ]
                )
    ...
    def estimate(
        ...
        if (
            self._quantum_instance is not None
            ...
            for j in range(1, self._maxiter + 1):
                ...
                if first_stage:
                    ...
                    theta_ci = [
                        np.arccos(x) / (2 ** (j + 1) + 2)
                        for x in chernoff_ci[::-1]
                    ]

                    if (
                        2 ** (j + 1) * theta_ci[1] >= 3 * np.pi / 8
                        and j < self._maxiter
                    ):
                        j_0 = j
                    ...
                    cos_2 = cos_estimate(
                        2 ** (j - 1) + 2 ** (j_0 - 1), self._shots[1]
                    )
                    sin = (cos * np.cos(v) - cos_2) / np.sin(v)
                    rho = np.arctan2(sin, cos)
                    n = int(
                        ((2 ** (j + 1) + 2) * theta_ci[1] - rho + np.pi / 3)
                        / (2 * np.pi)
                    )

                    theta_ci = [
                        (2 * np.pi * n + rho + sign * np.pi / 3)
                        / (2 ** (j + 1) + 2)
                        for sign in [-1, 1]
                    ]
        ...
        if (
            ...
            result.success_probability = (
                1 - (2 * self._maxiter - j_0) * self._delta
            )
