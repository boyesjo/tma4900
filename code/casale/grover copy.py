...
def grover(
    ...
    f_reg = QuantumRegister(1, name="f")
    o = QuantumCircuit(q_reg, f_reg).compose(
        oracle_f.circ(x_reg=q_reg, y_reg=[], f=f), q_reg
    )

    r = (
        QuantumCircuit(q_reg, f_reg)
    ...
    qc = QuantumCircuit(q_reg, f_reg)
    qc.h(q_reg)
    for _ in range(n):
        qc = qc.compose(o, q_reg[:] + f_reg[:]).compose(r, q_reg[:] + f_reg[:])
